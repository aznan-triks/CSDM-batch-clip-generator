...
class App(EngineStateMixin, EngineMixin, tk.Tk):
    ...
    def _connect_and_load(self):
        ...
        def task():
            try:
                data = self.discover_database()
                self.after(0, lambda: self._on_load_ok(data))
            except Exception as e:
                self.after(0, lambda err=e: self._on_load_fail(err))

        threading.Thread(target=task, daemon=True).start()

    def _on_load_ok(self, data):
        self.apply_discovery(data)
        # Everything below is widget work and stays here.
        self.db_status.set(
            f"[DB:OK] {len(data['players'])}P·{len(data['tags'])}T"
            + ("" if data["date_col"] else " ⚠date?"))
        ...
        self.player_search.set_players(data["players"], restore_steam_id=restore_sid)

        self._build_weapons(data["weapons"])
