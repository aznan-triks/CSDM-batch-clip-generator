...
### Fixed: three filter settings that no button could reach

**What changed:** Three saved settings existed in the config file but had no control anywhere in the window. FERRARI PEEK was missing its "Exclude" box, so there was no way to say "give me every kill *except* the moving-peek ones" even though the engine already knew how to do it. Two leftovers from an older version of the TROIS SHOT filter have been cleaned out; the "Exclude" box you already use on that row keeps working exactly the same, and old saved settings and presets are carried over automatically.

**Technical:**

- `kill_mod_high_velocity_exclude` was auto-generated by the `KILL_FILTER_REGISTRY` loop (`csdm/static_data.py`) and honoured by the dp2 exclusion pass (`_apply_dp2_modifiers`, `_apply_dp2_filters_to_events`) and by `_dp2_required_sections`, but never referenced in the UI: the FERRARI PEEK row was hand-built instead of going through `_build_filter_row`. The row now uses `_build_filter_row` like FLICK and ONE TAP, and its collapsible One-shot / Min-approach panel is stacked on the returned row. ~20 duplicated lines removed.
- The panel now follows the Enable variable via `trace_add` instead of a checkbox command, so it also collapses when Exclude, "Unselect all" or a preset clears the filter.
- `kill_mod_no_trois_shot` was unreachable: its checkbox lived in an `elif fdef.special == "trois_shot"` branch that `has_exclude` had made dead, since `kill_mod_trois_shot` was absent from `_NO_AUTO_EXCLUDE` and therefore got an auto-generated `_exclude` key. The generic exclusion path produces an identical result (it runs `_trois_shot_filter` and strips the matches), so the legacy mechanism was a duplicate. Removed: the registry entry, `_no_trois_shot_filter`, `_on_no_trois_shot_toggle`, `_on_trois_shot_toggle`, the dead `elif`, and the two engine branches. Its one unique behaviour — clearing TROIS TAP — is preserved by `_on_trois_shot_exclude`, wired through the new `exclude_cmd_map` in `_build_filter_row`.
- `kill_mod_no_trois_shot_req` was orphaned (`_req` keys are generated for every registry entry, with no `hide_ui` guard — `kill_mod_mate_pov_req` legitimately depends on that). It disappears with the registry entry.
- Migration: `migrate_legacy_filter_keys()` in `csdm/config.py` maps a truthy `kill_mod_no_trois_shot` onto `kill_mod_trois_shot_exclude` and drops `kill_mod_no_trois_shot_req`. Idempotent, applied by `_migrate_config` and by `_load_preset` on preset payloads.

### Removed

- `kill_mod_no_trois_shot`, `kill_mod_no_trois_shot_req` — see above. Do not reintroduce; the generic per-filter `_exclude` mechanism covers the same need.
